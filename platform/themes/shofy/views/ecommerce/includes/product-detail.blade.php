...
<div class="tp-product-details-inventory d-flex align-items-center mb-10">
    @if (is_plugin_active('marketplace') && $product->store?->id)
        <div class="tp-product-details-stock mb-10">
            <span><a href="{{ $product->store->url }}">{{ $product->store->name }}</a> {!! $product->store->badge !!}</span>
        </div>
    @endif

    @if (EcommerceHelper::isReviewEnabled() && ($product->reviews_avg || theme_option('ecommerce_hide_rating_star_when_is_zero', 'no') === 'no') && (!EcommerceHelper::hideRatingWhenNoReviews() || $product->reviews_count > 0))
...
<div class="tp-product-details-views mb-10 d-flex align-items-center gap-2">
...
<x-core::form :url="route('public.cart.add-to-cart')" method="POST" class="product-form">
    ...
    @if (isset($flashSale))
        <div class="tp-product-details-countdown justify-content-between flex-wrap mt-25 mb-25">
            ...
            <div class="tp-product-details-countdown-time" data-countdown data-date="{{ $flashSale->end_date->endOfDay() }}">
                <ul>
