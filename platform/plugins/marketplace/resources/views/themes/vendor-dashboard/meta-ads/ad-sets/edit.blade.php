        ...
        <div class="card-body">
            ...
            <form action="{{ route('marketplace.vendor.meta-ads.ad-sets.update', $adSet->id) }}" method="POST">
                ...
                @include(MarketplaceHelper::viewPath('vendor-dashboard.meta-ads.partials.interest-picker'), [
                    'existingInterests' => old('targeting_interests')
                        ? (json_decode(old('targeting_interests'), true) ?? [])
                        : collect($adSet->targeting_interests ?? [])->filter(fn ($i) => is_array($i) && ! empty($i['id']))->values()->all(),
                ])
                <div class="mb-3">
